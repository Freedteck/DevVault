...
async function fetchUserQuestions(accountId: string): Promise<LiveQuestion[]> {
  ...
  if (!topicId) return [];
  const res = await fetch(`${MIRROR}/topics/${topicId}/messages?limit=200`);
  if (!res.ok) return [];
  const j = await res.json();
...
export default function ProfilePage() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      setEditSkills(
        ((profilePayload?.skills as string[]) ?? []).join(", "),
      );
  ...
  if (!isConnected) {
    return (
      <div className="max-w-4xl mx-auto py-20 text-center space-y-4">
        <h1 className="text-2xl font-bold text-text-main">Your Vault Profile</h1>
        <p className="text-sm text-text-secondary">
          Connect your wallet to view and manage your on-chain developer profile.
        </p>
      </div>
    );
  }
