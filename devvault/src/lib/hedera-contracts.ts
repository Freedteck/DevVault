...
function keccak256Selector(sig: string): Buffer {
  ...
  const selectors: Record<string, string> = {
    "lockHbar(string,uint256)": "0x4e971571",
    "release(string,uint256,address)": "0x8f3aebfe",
    "cancel(string,uint256)": "0x84741e8c",
    "swap(string)": "0x5cae8005",
    "quote(uint256)": "0x00e4768b",
  };
...
export async function releaseBounty(
  ...
  const stringEncoded = encodeString(input.topicId);
  const callData = Buffer.concat([selector, offset, seqNum, addrBuf, stringEncoded]);
