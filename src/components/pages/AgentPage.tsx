...
const PROFILE_TOPIC_ID = process.env.NEXT_PUBLIC_AGENT_PROFILE_TOPIC_ID!;
const OPERATOR_ID = `${INBOUND_TOPIC_ID}@${AGENT_ACCOUNT_ID}`;

function getAgentStatus(activities: any[]) {
...
const HashscanLink = ({ path, label }: HashscanLinkProps) => (
  <a
    href={`${HASHSCAN}/${path}`}
    target="_blank"
...
const ActivityIcon = ({ type }: ActivityIconProps) => {
  ...
  if (type === "connection")
    return <Link2 size={14} className={styles.iconConnection} />;
  if (type === "hol_reply")
    return <Radio size={14} className={styles.iconHol} />;
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div
          className={`${styles.statusBadge} ${styles[`status_${status.color}`]}`}
        >
      ...
      <div className={styles.grid}>
        <GlassCard className={styles.identityCard}>
          ...
          <div className={styles.idRows}>
            <div className={styles.idRow}>
              ...
              <HashscanLink
                path={`account/${AGENT_ACCOUNT_ID}`}
                label={AGENT_ACCOUNT_ID}
              />
            </div>
            <div className={styles.idRow}>
              <span className={styles.idLabel}>Operator ID</span>
              <span
                className={`${styles.monoId} ${styles.operatorId}`}
                title="HCS-10 format: inboundTopicId@accountId"
              >
                {OPERATOR_ID}
              </span>
            </div>
            ...
            <div className={styles.idRow}>
              ...
              <HashscanLink
                path={`topic/${INBOUND_TOPIC_ID}`}
                label={INBOUND_TOPIC_ID}
              />
            </div>
            ...
            <div className={styles.idRow}>
              ...
              <HashscanLink
                path={`topic/${OUTBOUND_TOPIC_ID}`}
                label={OUTBOUND_TOPIC_ID}
              />
            </div>
            <div className={styles.idRowHint}>
            ...
            <div className={styles.idRow}>
              ...
              <HashscanLink
                path={`topic/${PROFILE_TOPIC_ID}`}
                label={PROFILE_TOPIC_ID}
              />
            </div>
        ...
        <div className={styles.rolesCol}>
          ...
          <GlassCard className={styles.roleCard}>
            <div className={styles.roleHeader}>
              ...
              <div>
                ...
                <span className={styles.roleTrigger}>
                  Triggered on question post · every 15 min
                </span>
              </div>
            </div>
            <p className={styles.roleDesc}>
              When a question is posted to DevVault, a GitHub Actions workflow
              triggers this agent. It reads the question from HCS, fetches the
              full content from IPFS, calls the Groq LLaMA-3.3-70B model, and
              posts an AI answer back to the HCS answers topic — but only when
              confidence is <strong>≥ 50%</strong>. Lower-confidence questions
              are left for humans.
            </p>
          </GlassCard>
          ...
          <GlassCard className={styles.roleCard}>
            <div className={styles.roleHeader}>
              <span
                className={`${styles.roleIconWrap} ${styles.roleIconArbitrate}`}
              >
                <Shield size={18} />
              </span>
              <div>
            ...
            <p className={styles.roleDesc}>
              For questions with HBAR bounties that have no accepted answer
              after <strong>7 days</strong>, the arbiter fetches all answers,
              scores each one with AI, picks the highest-quality response, and
              calls <code>arbiterRelease()</code> on the escrow smart contract —
              distributing the bounty automatically with no human intervention
              required.
            </p>
          </GlassCard>
          ...
          <GlassCard className={styles.roleCard}>
            <div className={styles.roleHeader}>
              ...
              <div>
                ...
                <span className={styles.roleTrigger}>
                  Polls inbound topic every 5 min
                </span>
              </div>
            </div>
            <p className={styles.roleDesc}>
              Registered in the Hashgraph Online Agent Registry (HCS-10
              OpenConvAI), this agent accepts <code>connection_request</code>{" "}
              messages from other agents and humans on its inbound HCS topic. On
              receiving a question via the HCS-10 message protocol, it generates
              an answer and delivers it directly back to the sender&apos;s
              return topic — enabling agent-to-agent knowledge exchange.
            </p>
          </GlassCard>
        </div>
      </div>
      ...
      <GlassCard className={styles.activityCard}>
        <div className={styles.activityHeader}>
          <div>
            ...
            <p className={styles.sectionHint}>
              Live feed from the agent&apos;s HCS-10 outbound topic (
              {OUTBOUND_TOPIC_ID})
            </p>
          </div>
          <NeonButton
