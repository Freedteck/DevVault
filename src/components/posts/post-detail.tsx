...
export function PostDetail({ post, onVote, className = "" }: PostDetailProps) {
  const { user } = useAuth();
    ...
    <Card className={`overflow-hidden ${className}`}>
      <CardHeader className="p-4 pb-0">
        <div className="flex justify-between items-start mb-2">
          ...
          <Badge variant={post.category === "QA" ? "default" : "secondary"} className="ml-2">
            {post.category === "QA" ? "Q&A" : "Resource"}
          </Badge>
        </div>
        
        <div className="flex items-center space-x-4 text-sm text-muted-foreground">
          <UserAvatar user={post.author} showName size="md" />
          <div className="flex items-center">
            ...
            <span>
              Posted on {format(new Date(post.createdAt), "MMM d, yyyy 'at' h:mm a")}
            </span>
          </div>
        </div>
      </CardHeader>
      ...
      <CardFooter className="p-4 flex flex-wrap justify-between gap-4">
        ...
        <div className="flex items-center space-x-2">
          <Button
            ...
            disabled={isSubmittingForAI || !user}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
            ...
            <span>{isSubmittingForAI ? "Analyzing..." : "Submit for AI Training"}</span>
          </Button>
          ...
          <Button
            ...
            disabled={!user || (user && user.id === post.authorId)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
            ...
            <span>
              Tip ({post.tipAmount} {TOKEN_SYMBOL})
            </span>
          </Button>
        </div>
      </CardFooter>
