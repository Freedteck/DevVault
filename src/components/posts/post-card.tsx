...
export function PostCard({ post, showFullContent = false }) {
  const contentPreview = showFullContent
    ? post.data.content
    : post.data.content.length > 200
    ? `${post.data.content.substring(0, 200)}...`
    : post.data.content;

  const postUrl = `/post/${post?.data.id}`;
  ...
  return (
    <Card className="overflow-hidden">
      <CardContent className="p-4 pb-2">
        ...
        <div className="text-sm text-muted-foreground mb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <UserAvatar user={post?.data.author} size="sm" />
              <span>
                Posted on{" "}
                {format(new Date(post?.data.createdAt || "0"), "MMM d, yyyy")}
              </span>
            </div>
        ...
        <div className="mb-4">
          {showFullContent ? (
            <RichTextContent content={contentPreview} className="prose-sm" />
          ) : (
