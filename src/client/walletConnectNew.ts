 ...
 */
async function initDAppConnector(
  openModal = true,
): Promise<DAppConnector | null> {
  if (dAppConnector) {
  ...
  if (!projectId) {
    throw new Error(
      "NEXT_PUBLIC_WALLET_CONNECT_PROJECT_ID is required in .env file",
    );
  }
...
async function walletConnectFcn(): Promise<DAppConnector | null> {
  console.log(`\n=======================================`);
