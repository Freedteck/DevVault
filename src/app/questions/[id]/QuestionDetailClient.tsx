...
interface QuestionDetailClientProps {
  ...
  summary?: LiveAnswer | null;
  autoAnswer?: LiveAnswer | null;
}
...
export function QuestionDetailClient({
  ...
  const handleSubmitReply = async (answerSequenceNumber: number) => {
    ...
    try {
      ...
      // Let the mirror node index it briefly
    ...
    try {
      ...
      if (question.bountyAmount > 0 && question.bountyCurrency === "HBAR") {
        ...
        const depositResult = await depositToAnswer(connector, {
          accountId,
          topicId: question.discussionTopicId,
          sequenceNumber: 0, // must match lockBounty
      ...
      // Let the mirror node index it briefly before we clear cache
      await new Promise((resolve) => setTimeout(resolve, 3000));
  ...
  return (
    <div className="max-w-5xl space-y-8 pb-12">
      ...
      {/* AI Auto Answer Insight Panel */}
      {autoAnswer && (
        <div className="mt-8 rounded-xl border border-violet-500/30 bg-violet-500/5 p-6 space-y-4">
          <div className="flex items-center gap-3 border-b border-violet-500/20 pb-3">
            <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-violet-600/20 text-violet-400 text-xs font-bold shrink-0">
              AI
            </span>
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-violet-400/80 uppercase tracking-widest font-semibold mt-0.5">
                Generated by Vurso AI
              </span>
            </div>
          </div>

          <MarkdownBody
            content={autoAnswer.body}
            className="text-sm text-text-secondary leading-relaxed"
          />

          <div className="pt-3 mt-4 border-t border-violet-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-violet-400/70">
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-violet-500/50" />
              Baseline generated because there is no bounty.
            </div>
            <span className="font-semibold text-violet-300">
              Have a better solution? Share your expertise below!
            </span>
          </div>
        </div>
      )}
          ...
          {answers.length > 0 ? (
            <>
              {answers.slice(0, visibleAnswers).map((answer) => {
                if (answer.isSpamFlag) return null;
                ...
                return (
                  <AnswerCard
                    ...
                    onSubmitReply={() =>
                      handleSubmitReply(answer.sequenceNumber)
                    }
                  />
                );
              })}
          ...
          <div className="rounded-md border border-border-main overflow-hidden bg-bg-panel">
            ...
            {question.bountyAmount > 0 &&
              question.bountyCurrency === "HBAR" && (
                <div className="flex items-start gap-3 px-4 py-3 border-b border-accent-500/20 bg-accent-500/5">
                  <span className="text-accent-400 shrink-0 mt-0.5" />
                  <div className="text-xs text-text-secondary leading-relaxed">
                    <span className="font-semibold text-accent-400">
                      Bounty question — human experts only.
                    </span>{" "}
                    ...
                    <span className="text-text-muted">
                      Accepted answer → bounty + deposit back. Valid non-winning
                      answer → deposit fully refunded. Spam or AI-generated
                      answer → deposit slashed.
                    </span>
                  </div>
                </div>
              )}
      ...
      {/* Mobile/Tablet Fallback: Render inline since right sidebar is hidden */}
      {summary && (
        <div className="xl:hidden mt-8 w-full">
          <div className="rounded-lg border border-border-main bg-bg-panel p-5">
            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-border-main/50">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] uppercase tracking-wider text-violet-400 font-semibold mt-0.5">
                  Verified Knowledge
                </span>
              </div>
            </div>
      ...
      {/* Desktop Portal: Teleport to DashboardLayout's right sidebar */}
      {summary &&
        mounted &&
        document.getElementById("thread-summary-portal") &&
        createPortal(
          <div className="rounded-lg border border-border-main bg-bg-panel p-5">
            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-border-main/50">
              <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-violet-600/20 text-violet-400 text-[10px] font-bold">
