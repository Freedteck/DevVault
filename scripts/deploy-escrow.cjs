...
async function main() {
  ...
  if (
    ...
    throw new Error(
      "Missing NEXT_PUBLIC_MY_ACCOUNT_ID or NEXT_PUBLIC_MY_PRIVATE_KEY in .env.local",
    );
  }
  ...
  if (!fs.existsSync(bytecodePath)) {
    console.error("❌ Bytecode not found. Run: npm run compile:escrow first");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`NEXT_PUBLIC_ESCROW_CONTRACT_ID=${contractId}\n`);
