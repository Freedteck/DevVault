...
async function main() {
  const accountIdStr = process.env.NEXT_PUBLIC_MY_ACCOUNT_ID;
  const privateKeyStr = process.env.NEXT_PUBLIC_MY_PRIVATE_KEY;
  const contractIdStr = process.env.NEXT_PUBLIC_ESCROW_CONTRACT_ID;

  if (!accountIdStr || !privateKeyStr || !contractIdStr) {
    throw new Error(
      "Missing NEXT_PUBLIC_MY_ACCOUNT_ID, NEXT_PUBLIC_MY_PRIVATE_KEY, or NEXT_PUBLIC_ESCROW_CONTRACT_ID in .env.local",
    );
  }
